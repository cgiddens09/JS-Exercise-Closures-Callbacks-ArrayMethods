...
function processDuplicateFree(list, callback) {
  return callback(list.filter((thing,index) => list.indexOf(thing) === index));
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  const sizer = runners.filter((runner) => runner.shirt_size === tShirtSize);
  return sizer;
}
...
function tallyUpDonations(runners) {
  const donate = runners.reduce((accum, runner) => {
